...
**🔴 硬约束：Bot 发消息永远无法产生蓝色 @ 链接**

Hermes Gateway 的飞书 Bot 走的是回复流程（而非新消息 API），因此发出的 `@名字` 是纯文本，不会被飞书渲染为蓝色 @ 链接。这不是操作失误，是平台技术限制。

**正确做法：**
- **Bot 间指代：** 直接用文字说出名字，如「极客 Claude 请处理这个问题」。其他 Bot 应主动检查上下文，发现被提到时主动回应
- **人类用户 @ Bot：** 在飞书客户端输入 `@` 并从弹出列表中选择，才能产生有效蓝链
- **关键节点串联：** 由人类用户做蓝链 @ 的手动串联，Bot 之间用文字指代
