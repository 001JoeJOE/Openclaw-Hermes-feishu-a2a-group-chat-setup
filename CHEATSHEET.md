...
★ 飞书原生 @所有人 对 Bot 无效——不会触发消息事件。需用约定关键词「所有人」或检测 raw JSON 中的 `@_all`。Hermes 已内置支持（`FEISHU_GROUP_POLICY=open` + `require_mention=true`），Bridge 需加代码检测 `rawContent.includes('@_all')`，OpenClaw 需自定义配置（`monitor-DDkD5r4p.js` 内置 `@_all` + `所有人` 检测但需额外启用）。
